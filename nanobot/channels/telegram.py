    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            try:
                ...
                if "Message is too long" in str(e2):
                    await self._send_long_message(int(msg.chat_id), html_content, msg.content)
                else:
                    logger.error(f"Error sending Telegram message: {e2}")

    async def _send_long_message(self, chat_id: int, html_content: str, text_content: str) -> None:
        summary_limit = 1000
        summary = text_content[:summary_limit].rstrip()
        if len(text_content) > summary_limit:
            summary = f"{summary}\n\n[Response truncated, full HTML attached]"
        summary_html = _markdown_to_telegram_html(summary)

        try:
            await self._app.bot.send_message(chat_id=chat_id, text=summary_html, parse_mode="HTML")
        except Exception as e:
            logger.warning(f"Failed to send summary message: {e}")

        file_bytes = html_content.encode("utf-8")
        file_obj = io.BytesIO(file_bytes)
        file_obj.name = "response.html"
        try:
            await self._app.bot.send_document(
                chat_id=chat_id,
                document=file_obj,
                filename="response.html",
                caption="Full response (HTML)",
            )
        except Exception as e:
            logger.error(f"Error sending Telegram HTML attachment: {e}")
